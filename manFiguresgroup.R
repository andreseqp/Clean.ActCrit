...
plot_grid(nrow = 2,obs.both,pred.both)
...
obs.pred.both<-ggplot(data = fieldatabyLoc.joint, 
                      aes(x=probvisitor.pred,y=probVisi.data,color=competence))+
...
obs.pred.both<-ggplot(data = fieldatabyLoc.both.1, aes(x=probvisitor.pred,y=probVisi.data))+
  geom_point(data = fieldatabyLocSamps.both,aes(x=probvisitor.pred,y=probVisi.data),
             color='grey')+
  geom_point()+theme_classic()+geom_abline(slope = 1)+
  lims(x=c(0.45,0.8),y=c(0.45,0.8))+
...
FIAinterpData.both<-AbundLeavData2interp(FIAlastQuarData[Gamma>0.7],
                                    Var2int = "Prob.RV.V",npoints = pointInt)

FIAinterpData.both.1<-AbundLeavData2interp(FIAlastQuarData[Gamma<0.7],
                                         Var2int = "Prob.RV.V",npoints = pointInt)


colorbreaksMeans<-seq(0,1,length=100)


# simsDir <- paste0("e:/NeuchSims/AC/",paste0(scenario,"_"))
# Interpolation for second model -----------------------------------------------
...
par(plt=c(posPlot(numplotx = 7,idplotx = 1)[1],
          posPlot(numplotx = 7,idplotx = 3)[2:4]))
...
#                                          colorbreaksMeans)]
         # ,cex=3,pch=20)
with(fieldatabyLoc.both[competence==0],{
points(x=re.abund.clean,y = prob.Vis.leave,
       col=paletteMeans(100)[findInterval(probVisi.data,
                                          colorbreaksMeans)]
       ,cex=3,pch=20)
points(x=re.abund.clean,y = prob.Vis.leave,
       col="black"
...
with(fieldatabyLoc.both[competence==1],{
  points(x=re.abund.clean,y = prob.Vis.leave,
         col=paletteMeans(100)[findInterval(probVisi.data,
                                            colorbreaksMeans)]
         ,cex=3,pch=20)
...
color.bar.aeqp(paletteMeans(100),min =0,#round(min(Prob.RV.V),2),
               max = 1,#round(max(Prob.RV.V),2),
               nticks = 3,
...
names(FIAinterpData.both.1)<-c("rel.abund.cleaners","prob.Vis.Leav","market_binomial_data")
names(fieldatabyLoc.both)[c(5,6,3)]<-c("rel.abund.cleaners","prob.Vis.Leav","market_binomial_data")
...
cont.obs.pred.both<- ggplot(data = FIAinterpData.both,aes(x=rel.abund.cleaners,y=prob.Vis.Leav,
 ...
 geom_raster(interpolate = TRUE) +  
  ...
  geom_point(data = fieldatabyLoc.both[competence==0],aes(fill=market_binomial_data),size=5,
...
scatter.obs.pred.both<-ggplot(data = fieldatabyLoc.both[competence==0],
                              aes(y=market_binomial_data,x=probvisitor.pred))+
  # geom_point(data = fieldatabyLocSamps.both,aes(x=probvisitor.pred,y=probVisi.data),
  #            color='grey')+
  geom_point(size=4)+ylim(0.3,0.9)+xlim(0.3,0.9)+
...
cont.obs.pred.1<- ggplot(data = FIAinterpData.both.1,aes(x=rel.abund.cleaners,y=prob.Vis.Leav,
                                                          fill=market_binomial_data))+
  geom_raster(interpolate = TRUE) +  
  scale_fill_gradientn(limits=c(0.3,1),colours= myPalette(100))+theme_classic()+
  geom_point(data = fieldatabyLoc.both[competence==1],aes(fill=market_binomial_data),size=5,
...
scatter.obs.pred.1<-ggplot(data = fieldatabyLoc.both[competence==1],aes(y=market_binomial_data,x=probvisitor.pred))+
...
cont.obs.pred.1,scatter.obs.pred.1,
