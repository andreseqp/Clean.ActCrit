...
a Metropolis sampler. ABC is a statistical technique to fit parameters of complicated models to data when we can't explicitely compute the likelihood as a function of the model parameters. Instead, we can compare the outcome of the model, given a set of parameters, to the actual data usually using a set of summary statistics that summarize the fit of the model predictions to the data [@beaumont_Approximate_2010a]. Thus, individual-based models such as the ones developed under the framework of reinforcement learning would be amenable to fit with this technique. In here, we will use the data from @triki_Biological_2019 to fit the parameters of the  model analyzed in @quinones_Reinforcement_2019. 
