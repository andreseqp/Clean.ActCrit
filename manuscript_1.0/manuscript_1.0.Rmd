---
title: Combination of field and experimental data together with computational models 
  reveal cognitive mechanisms behind cleaner fish behaviour
#Cognitive and ecological aspects determine cleaner performance
author:
...
corresp_author_name:  "AE. Quiñones"
corresp_author_email: "andreseqp@gmail.com"
...
author_contributions: |
  AQ and RB designed the study. ZT collected the data. AQ developed the model and 
  fitted the model parameters to the data. AQ, RB and ZT wrote the manuscript. 

## Remove this if not required
conflict_of_interest: |
  The authors declate no conflict of interests.

## Remove this if not required
funding: |
  This work was supported by the Swiss National Science Foundation (grants
  number: 31003A_153067/1 and 310030B_173334/1 to R.B.).


## Remove this if not required
acknowledgements: |
  ZT and RB kindly thank the staff of Lizard Island Research Station, Dominique
...
[@hammerstein_Biological_2016; @leimar_Cooperation_2010;
...
correlated with parasite load [@grutter_Relationship_1995a]. However, 
early laboratory
experiments showed that leaving causes service priority independent of
client value [@bshary_Asymmetric_2002]. The experiments consisted of a
...
[@wismer_Cuebased_2019]), or whether the information is in the colour of
...
made choices on a touch screen [@pretot_Factors_2016]. Second, it
...
are more likely to fail to complete the task [@triki_Biological_2019;
@triki_Decrease_2018; @wismer_Variation_2014]. The apparent explanation
...
cleaner fish densities are low [@triki_Brain_2020]. This change in
...
low-density areas [@triki_Brain_2020], on a local scale those
...
high/low density) have relatively larger forebrains [@triki_Brain_2020].
...
decision-making [@shettleworth_Cognition_2009].
...
[@dubois_Model_2021]). By varying the cognitive tool kit, one can assess
which minimal kit is necessary to solve the task at hand. This approach
has been used to analyse the market task [@prat_Role_2021;
@quinones_Reinforcement_2019]. Learning models showed that basic
...
strategic option) [@quinones_Reinforcement_2019]. In addition, cleaners
...
[@quinones_Reinforcement_2019]: through low temporal discounting of
...
operant conditioning [@thorndike_Animal_1898; @skinner_Behavior_1938], 
...
[@suddendorf_Evolution_2007]. Here, we fitted our model to the empirical
...
We used the data from [@triki_Biological_2019] to fit the parameters of
the model analysed in [@quinones_Reinforcement_2019]. For the bayesian
...
found in the paper [@quinones_Reinforcement_2019]. The model analysis
...
effects of their choices. The model analysis by Quiñones *et al*
[@quinones_Reinforcement_2019] shows that without either of them, learning
...
supported by Triki *et al* [@triki_Biological_2019] data. However, a combination of
...
the $pseudo-R^2$ [@mcfadden_Conditional_1974], which is a standard
...
the market task [@triki_Biological_2019]. For both mechanisms it is
...
decision-rules [@triki_Brain_2020]. This suggests that despite the
overall behavioural adjustment, evidenced here and elsewhere
[@triki_Biological_2019; @triki_Decrease_2018], there are some
cleaners better than others at adjusting their behaviour. Triki *et al.*
[@triki_Brain_2020] refer to these as socially competent cleaners. Social
competence refers to the ability to optimise social behaviour depending
on the available social information [@taborsky_Social_2012a;
@bshary_Cooperation_2015]. However, we have shown here that cleaners
...
the ones reported in Triki *et al.* [@triki_Brain_2020], is configurational learning or
...
station [@quinones_Reinforcement_2019]. The latter is referred to as
configurational learning or chunking [@sutherland_Configural_1989;
@miller_Magical_1956]. Theoretical models suggest that the lack of
configurational learning is related to a low frequency with which
stimuli are encountered together [@kolodny_Evolution_2014;
@prat_Role_2021]. Thus, cleaner relative abundance could be related to
this mechanisms as well. Given that the lack of either of these two
mechanisms yields neutral preferences [@quinones_Reinforcement_2019],
...
[@woodbury_Learning_1943; @sutherland_Configural_1989].
